...
def test_multi_subject_selection_creates_one_job_and_one_paper_per_subject(
    ...
    assert set(jobs_by_subject.keys()) == {algo["id"], dbms["id"]}
    ...
    assert "Algorithms" in jobs_by_subject[algo["id"]].title
    assert "Databases" in jobs_by_subject[dbms["id"]].title
...
def test_unauthenticated_request_cannot_create_a_job(drive):
    ...
    response = client.post(
        f"{UI}/new",
        data={
            "exam_id": "1", "department_id": "1", "subject_ids": ["1"],
...
def test_unauthenticated_request_cannot_update_usage():
    response = client.post(f"{UI}/999999/usage", data={"usage_flags": []}, follow_redirects=False)
    assert response.status_code == 303
    assert response.headers["location"] == "/admin/login"
...
def test_unknown_paper_id_redirects_instead_of_erroring(admin_auth_headers):
    response = client.get(f"{UI}/papers/9999999/usage", headers=admin_auth_headers, follow_redirects=False)
    assert response.status_code == 303
